...
class _PostViewState extends State<_PostView> {
  ...
  static const _ageLimits = ['0+', '6+', '12+', '16+', '18+'];
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // ── Age Limit ─────────────────────────
                      _buildSectionLabel('Age Limit'),
                      const SizedBox(height: 12),
                      _buildAgeLimitSelector(bloc),
                      const SizedBox(height: 24),
  ...
  Widget _buildVideoUpload(PostBloc bloc) {
    return BlocBuilder<PostBloc, PostState>(
      ...
      builder: (context, state) {
        final isLoading =
            state is PickVideoState && state.state == BaseState.loading;
  ...
  Widget _buildPickedVideoContent(String path) {
    return Column(
      ...
      children: [
        ...
        Text(
          'Tap to change',
          style: TextStyle(color: ColorName.accent, fontSize: 12),
        ),
      ],
    );
  }
  ...
  Widget _buildThumbnailUpload(PostBloc bloc) {
    return BlocBuilder<PostBloc, PostState>(
      ...
      builder: (context, state) {
        final isLoading =
            state is PickThumbnailState && state.state == BaseState.loading;
                ...
                : Row(
                    children: [
                      Container(
                        ...
                        child: isPicked
                            ? Image.asset(bloc.thumbnailPath!,
                                fit: BoxFit.cover)
                            : Column(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
              ...
              Expanded(
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: hint,
                    hintStyle: TextStyle(
                        color: ColorName.contentSecondary, fontSize: 13),
  ...
  Widget _buildCategoryDropdown(PostBloc bloc) {
    return BlocBuilder<PostBloc, PostState>(
      ...
      builder: (context, state) {
        final isLoading = state is FetchCategoriesState &&
        ...
        return GestureDetector(
          onTap:
              isLoading ? null : () => _showCategoriesSheet(context, bloc),
          child: Container(
            height: 52,
            decoration: BoxDecoration(
              color: ColorName.backgroundSecondary,
            ...
            child: Row(
              children: [
                ...
                Icon(Icons.keyboard_arrow_down_rounded,
                    color: ColorName.contentSecondary, size: 20),
                const SizedBox(width: 14),
              ],
            ),
          ),
                ...
                itemBuilder: (_, index) {
                  ...
                  return ListTile(
                    title: Text(
                      ...
                      style: TextStyle(
                        color:
                            isSelected ? ColorName.accent : Colors.white,
  ...
  //  AGE LIMIT SELECTOR
  // ─────────────────────────────────────────
  Widget _buildAgeLimitSelector(PostBloc bloc) {
    return BlocBuilder<PostBloc, PostState>(
      buildWhen: (_, state) => state is SelectAgeLimitState,
      builder: (context, state) {
        final selected = bloc.selectedAgeLimit;
        return Row(
          children: _ageLimits.map((age) {
            final isActive = age == selected;
            return GestureDetector(
              onTap: () =>
  ...
  Widget _buildSubmitButton(BuildContext context, PostBloc bloc) {
    return BlocBuilder<PostBloc, PostState>(
      ...
      builder: (context, state) {
        final isLoading =
            state is CreatePostState && state.state == BaseState.loading;
          ...
          onTap: isLoading
              ...
              : () {
                  bloc.add(CreatePostEvent(
                    ...
                    ageLimit:      bloc.selectedAgeLimit,
                  ));
                },
          child: Container(
            height: 52,
            decoration: BoxDecoration(
              color: isLoading
                  ? ColorName.accent.withValues(alpha: .5)
                  : ColorName.accent,
              borderRadius: BorderRadius.circular(14),
            ),
            child: isLoading
